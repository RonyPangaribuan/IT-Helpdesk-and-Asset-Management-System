<x-app-layout>
    <x-slot name="header">
        <x-page-header
            ...
            description="Archived categories remain visible on existing tickets but cannot be selected for new records."
        >
            <x-slot name="actions">
                <a href="{{ route('admin.ticket-categories.create') }}" class="app-button-primary">Create Category</a>
            </x-slot>
        </x-page-header>
    </x-slot>
                ...
                <table class="app-table">
                    <thead class="app-table-head">
                        <tr>
                            ...
                            <th class="app-table-th" scope="col">Tickets</th>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse ($categories as $category)
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-4 py-4 text-sm text-slate-700">{{ $category->tickets_count }}</td>
                                <td class="px-4 py-4">
                                ...
                                <td class="px-4 py-4 text-right text-sm">
                                    @if (! $category->trashed())
                                        <div class="flex justify-end gap-3">
                                            <a href="{{ route('admin.ticket-categories.edit', $category) }}" class="app-link">Edit</a>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-4 py-6">
                                    <x-empty-state title="No ticket categories found." description="Create categories to organize incoming support requests." />
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
