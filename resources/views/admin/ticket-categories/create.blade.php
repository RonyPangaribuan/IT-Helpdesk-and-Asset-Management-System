<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h1 class="text-2xl font-semibold text-stone-950">Create Ticket Category</h1>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <div class="rounded-lg border border-stone-200 bg-white p-6 shadow-sm">
                <form method="POST" action="{{ route('admin.ticket-categories.store') }}" class="space-y-5">
                    @csrf

                    <div>
                        <x-input-label for="name" value="Name" />
                        <x-text-input id="name" name="name" class="mt-1 block w-full" value="{{ old('name') }}" required maxlength="100" />
