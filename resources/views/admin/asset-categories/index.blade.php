    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <h1 class="text-2xl font-semibold text-stone-950">Asset Categories</h1>
            </div>
            <a href="{{ route('admin.asset-categories.create') }}" class="inline-flex w-fit items-center rounded-md bg-teal-700 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-teal-800">
                Create Category
            </a>
        </div>
    </x-slot>
                    ...
                    <table class="min-w-full divide-y divide-stone-200">
                        <thead class="bg-stone-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-semibold uppercase text-stone-600">Assets</th>
                        ...
                        <tbody class="divide-y divide-stone-200 bg-white">
                            @forelse ($categories as $category)
                                <tr>
                                    ...
                                    <td class="px-4 py-4 text-sm text-stone-700">{{ $category->assets_count }}</td>
                                    <td class="px-4 py-4">
                                    ...
                                    <td class="px-4 py-4 text-right text-sm">
                                        @if (! $category->trashed())
                                            <div class="flex justify-end gap-3">
                                                <a href="{{ route('admin.asset-categories.edit', $category) }}" class="font-medium text-teal-700 hover:text-teal-900">Edit</a>
