...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>Server Error - {{ config('app.name', 'DelDesk') }}</title>
    ...
    <body class="bg-slate-50 font-sans text-slate-900 antialiased">
        <main class="flex min-h-screen items-center justify-center px-4 py-12">
            <section class="w-full max-w-lg rounded-lg border border-slate-200 bg-white p-8 text-center shadow-sm">
                ...
                <h1 class="mt-3 text-4xl font-semibold text-slate-950">500</h1>
                <p class="mt-3 text-lg font-medium text-slate-900">Something went wrong</p>
                <p class="mt-2 text-sm leading-6 text-slate-600">Please try again later or contact the system administrator.</p>
