<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <h1 class="text-2xl font-semibold text-stone-950">Edit Asset</h1>
        </div>
    </x-slot>
                ...
                <form method="POST" action="{{ route('assets.update', $asset) }}" class="space-y-5">
                    ...
                    <div class="grid gap-5 md:grid-cols-2">
                        <div>
                            ...
                            <x-text-input id="asset_code" name="asset_code" class="mt-1 block w-full" value="{{ old('asset_code', $asset->asset_code) }}" required maxlength="50" />
                            <x-input-error :messages="$errors->get('asset_code')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="name" value="Name" />
                            <x-text-input id="name" name="name" class="mt-1 block w-full" value="{{ old('name', $asset->name) }}" required maxlength="150" />
                    ...
                    <div class="grid gap-5 md:grid-cols-3">
                        <div>
                            ...
                            <x-text-input id="brand" name="brand" class="mt-1 block w-full" value="{{ old('brand', $asset->brand) }}" maxlength="100" />
                            <x-input-error :messages="$errors->get('brand')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="model" value="Model" />
                            <x-text-input id="model" name="model" class="mt-1 block w-full" value="{{ old('model', $asset->model) }}" maxlength="100" />
                            <x-input-error :messages="$errors->get('model')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="serial_number" value="Serial Number" />
                            <x-text-input id="serial_number" name="serial_number" class="mt-1 block w-full" value="{{ old('serial_number', $asset->serial_number) }}" maxlength="100" />
                    ...
                    <div>
                        ...
                        <x-text-input id="location" name="location" class="mt-1 block w-full" value="{{ old('location', $asset->location) }}" required maxlength="150" />
                    ...
                    <label class="inline-flex items-center gap-2 text-sm text-stone-700">
                        ...
                        <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 text-teal-700 shadow-sm focus:ring-teal-600" @checked((bool) old('is_active', $asset->is_active))>
                        Active
                    </label>
                    <x-input-error :messages="$errors->get('is_active')" class="mt-2" />

                    <div class="flex items-center justify-end gap-3">
                        <a href="{{ route('assets.show', $asset) }}" class="text-sm font-medium text-stone-600 hover:text-stone-950">Cancel</a>
